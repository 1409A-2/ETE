...
!function (a, b) {
        ...
        $(".workForm").each(function () {
            $(this).validate({
                rules: {
                    ...
                    workName: {required: !1, checkUrlNot: !0, maxlength: 150},
                    workDescription: {required: !1, maxlength: 100}
                },
                messages: {
                    workLink: {checkUrlNot: "请输入有效的作品链接", maxlength: "请输入有效的作品链接"},
                    workName: {checkUrlNot: "请输入有效的作品名称", maxlength: "请输入有效的作品名称"},
                    workDescription: {maxlength: "请输入100字以内的有效作品说明"}
                },
                submitHandler: function (a) {
                    var b = "请输入作品链接" != $('input[name="workLink"]', a).val() ? $('input[name="workLink"]', a).val() : "", c = "请输入说明文字" != $('textarea[name="workDescription"]', a).val() ? $('textarea[name="workDescription"]', a).val() : "", d = $("#resumeId").val(), e = $("#resumeId").val(), f = $("#token_work").val();
                    $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
                        url: "worksAdd",
                        type: "POST",
                        ...
                        $("#resubmitToken").val(b.resubmitToken),
                            b.success ? (c = "",
                                d = "",
                                e = b.content.workShow, 3 == b.code ? "" != e ? (c += '<li data-id="' + e.id + '"><div class="workList c7">' + '<i class="sm_del dn"></i>' + '<i class="sm_edit dn"></i>', e.url && (d = "http://" == e.url.substring(0, 7) || "https://" == e.url.substring(0, 8) ? e.url : "http://" + e.url,
                            ...
                            e.url && (d = "http://" == e.url.substring(0, 7) || "https://" == e.url.substring(0, 8) ? e.url : "http://" + e.url,
                                c += '<div class="f16">网址：<a href="' + d + '" target="_blank">' + e.url + "</a></div>"),
                                c += "<p>" + e.workName + "</p>" + "</div>", placeholderFn(),
                            ...
                            "" == $.trim(h.obj.children(".workShow").children(".slist").html()) && h.AddCancel),
                                ...
                                scoreChange(b.content.infoCompleteStatus.score), k(h.obj),
    ...
    window.onbeforeunload = function () {
        ...
        submitHandler: function (a) {
            var b = $('input[name="name"]', a).val(), c = $('input[name="gender"]:checked', a).val(), d = $('input[name="topDegree"]', a).val(), e = $('input[name="workyear"]', a).val(), f = $('input[name="currentState"]', a).val(), g = $('input[name="email"]', a).val(), h = $('input[name="tel"]', a).val(), i = $("#pageType").val(), j = $("#resumeId").val(),
                l = $("#resubmitToken").val();
            $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
                url: "educationPro",
                type: "POST",
                ...
                var c, d, e;
                //$("#resubmitToken").val(b.resubmitToken), b.success ? "educationPro" == b.content.url ? window.location.href = ctx + b.content.url : (c = b.content.resume, d = c.workYear, "应届毕业生" != d ? (d += "工作经验", $("#workExperience h2 span").text("（投递简历时必填）")) : $("#workExperience h2 span").text(""), e = c.name, "" != c.sex && (e += " | " + c.sex), "" != c.highestEducation && (e += " | " + c.highestEducation), e += " | " + d + "<br />" + c.phone + " | " + c.email + "<br />" + c.status, $(".collapsible_menu dt").html(c.name + "<i></i>"), $("#basicInfo .basicShow span").html(e).parent().removeClass("dn"), $("#basicInfo .basicEdit").addClass("dn"), $("#basicInfo .c_edit").removeClass("dn"), $("#nameVal").val(c.name), $("#genderVal").val(c.sex), $("#topDegreeVal").val(c.highestEducation), $("#workyearVal").val(c.workYear), $("#currentStateVal").val(c.status), $("#emailVal").val(c.email), $("#telVal").val(c.phone), $("#lastChangedTime span").text(b.content.refreshTime), b.content.isNew && changeAllIds(b.content.jsonIds), scoreChange(b.content.infoCompleteStatus.score), k($("#basicInfo"))) :  $(a).find(":submit").val("保 存").attr("disabled", !1)
            }
        }
        ...
        submitHandler: function (a) {
            var c = $('input[name="expectCity"]', a).val(), d = $('input[name="type"]:checked', a).val(), e = "期望职位，如：产品经理" != $('input[name="expectPosition"]', a).val() ? $('input[name="expectPosition"]', a).val() : "", f = $('input[name="expectSalary"]', a).val(), g = $("#resumeId").val(), h = $("#resubmitToken").val();
            $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
        ...
        rules: {selfDescription: {required: !1}}, submitHandler: function (a) {
            var b = $('textarea[name="selfDescription"]', a).val(), c = $("#resumeId").val(), d = $("#desc_token").val();
            $(a).find(":submit").val("保存中...").attr("disabled", !0), $.ajax({
                url: "educationDesc",
                type: "POST",
