...
$(function () {
    function a() {
        ...
        $("#jobForm").valid() && ($("#department").val() == $("#department").attr("placeholder") && $("#department").val(""), ("" == $("#salaryMin").val() || $("#salaryMin").val() == $("#salaryMin").attr("placeholder")) && $("#salaryMin").val(""), ("" == $("#salaryMax").val() || $("#salaryMax").val() == $("#salaryMax").attr("placeholder")) && $("#salaryMax").val(""), $("#positionAddress").val() == $("#positionAddress").attr("placeholder") && $("#positionAddress").val(""), a = $("#positionType").val(), b = $("#positionName").val(), c = $("#department").val(), d = $('#jobForm input[name="jobNature"]:checked').val(), e = $("#salaryMin").val(), f = $("#salaryMax").val(), g = $("#workAddress").val(), h = $("#experience").val(), i = $("#education").val(), j = $("#positionAdvantage").val(), k = $("#positionDetail").val(), l = $("#positionAddress").val(), m = $("#lng").val(), n = $("#lat").val(), o = $("#receiveEmail").val(), p = $("#forwardEmail").val(), q = $('#jobForm input[name="id"]').val(), r = $('#jobForm input[name="companyId"]').val(), s = $("#resubmitToken").val(), c_id = $("#c_id").val(), positionDetail = ue.getContent(),
            ...
            data: {
                ...
                if(a==1){
                    location.href="postOffice_issue";
                }else if(a==2){
                    alert("今日添加职位已满");
                }else{
                    alert("添加失败");
                }
            //$("#resubmitToken").val(a.resubmitToken), a.success ? window.location.href = '' : "" != a.content && null != a.content ? window.location.href = ctx + a.content : $("#beError").html(a.msg).show()
        }))
    }
    ...
    $("#telForm").validate({
        ...
        d = ue.getContent().length;
        return d >= 20 && 4e3 >= d ? !0 : !1
