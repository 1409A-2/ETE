...
/*!common/static/js/intro_base.js*/
;
define("common/static/js/intro_base", ["require", "exports", "module"], function () {
    $(function () {
        var h = {
            curIndex: 0,
            wrapper: null,
            ...
            slideHeight: 0,
            slideNumber: 0,
            direction: {LEFT: 998, RIGHT: 999},
            slideTime: 1e3,
            slideInterval: 6e3,
            intervalId: null,
            init: function () {
                var h = this.wrapper = $("#slides");
                this.content = h.find("ul.slide-content"), this.indicators = h.find("div.slide-indicator").find("span"), this.buttonLeft = h.find("button.left"), this.buttonRight = h.find("button.right");
                var a = this.slides = h.find("li.slide");
                this.slideWidth = a.width(), this.slideNumber = a.length, this.curIndex = 0, $(this.slides[this.slides.length - 1]).css({
                    position: "relative",
                    left: -1 * this.slides.length * this.slideWidth + "px"
                }), this.initContentSize(), this.initEvent(), this.initAutoPlay()
            },
            initButton: function () {
                0 == this.curIndex && this.buttonLeft.hide(), this.curIndex == this.slideNumber.length - 1 && this.buttonRight.hide()
            },
            initContentSize: function () {
                this.content.width(this.slideNumber * this.slideWidth)
            },
            initEvent: function () {
                var h = this;
                this.buttonLeft.on("click", function () {
                    h.slideTo(h.direction.LEFT)
                }), this.buttonRight.on("click", function () {
                    h.slideTo(h.direction.RIGHT)
                })
            },
            startTime: 0,
            slideTo: function (h) {
                var a = new Date;
                if (a - this.startTime < this.slideTime)return !1;
                this.startTime = a;
                var c = 0, v = this.curIndex, y = this.slideNumber, w = this.direction;
                switch (h) {
                    case w.LEFT:
                        c = v - 1, -1 == c && $(this.slides[y - 1]).css({
                            position: "relative",
                            left: -1 * y * this.slideWidth + "px"
                        });
                        break;
                    case w.RIGHT:
                        c = v + 1, c == y ? $(this.slides[0]).css({
                            position: "relative",
                            left: c * this.slideWidth + "px"
                        }) : c == y - 2 && $(this.slides[y - 1]).css({position: "relative", left: 0});
                        break;
                    default:
                        c = h
                }
                var g = this;
                this.content.animate({left: -c * this.slideWidth}, this.slideTime, function () {
                    (0 == c || c == y - 1) && ($(g.slides[c]).css({
                        position: "relative",
                        left: 0
                    }), g.content.css({left: -1 * c * g.slideWidth + "px"}))
                }), c == y ? c = 0 : -1 == c && (c = y - 1), this.updateIndicator(v, c), this.curIndex = c, this.initAutoPlay()
            },
            initAutoPlay: function () {
                null !== this.intervalId && clearInterval(this.intervalId);
                var h = this;
                this.intervalId = setInterval(function () {
                    h.buttonRight.trigger("click")
                }, this.slideInterval)
            },
            updateIndicator: function (h, a) {
                a == this.slides.length, $(this.indicators.get(h)).removeClass("active"), $(this.indicators.get(a)).addClass("active")
            }
        }, a = {
            curIndex: 0, wrapper: null, init: function () {
                this.wrapper = $("#tabs"), this.curIndex = 0, this.bindEvent()
            }, bindEvent: function () {
                var h = this;
                this.wrapper.on("click", "dt", function () {
                    h.changeContent($(this))
                }), this.initBtnMore()
            }, initBtnMore: function () {
                var h = 4, a = $(".questions").find("dt"), c = parseInt(a.css("line-height")), v = h * c + "px", y = a.length * c + "px", w = !1;
                $("#btn_more").on("click", function () {
                    w ? ($(this).removeClass("active"), $(this).prev().animate({height: v}), w = !1) : ($(this).addClass("active"), $(this).prev().animate({height: y}), w = !0)
                })
            }, changeContent: function (h) {
                var a = this.wrapper.find("dd"), c = this.wrapper.find("dt"), v = this.wrapper.find("dt").index(h);
                $(c.get(this.curIndex)).removeClass("active"), $(a.get(this.curIndex)).removeClass("active"), h.addClass("active"), $(a.get(v)).addClass("active"), this.curIndex = v
            }
        }, c = function (h) {
            function a(h) {
                b.css({display: "block", width: "0px", height: "0px"}), b.animate({
                    width: "944px",
                    height: "531px"
                }, 1e3, function () {
                    h && h()
            ...
            function c() {
                w.hide(), g.hide(), $("html").css({overflow: "auto"}), window.myPlayer ? myPlayer.pause() : T.close()
            }

            function v() {
                w.show(), g.show(), $("html").css({overflow: "hidden"}), window.myPlayer || T.open(), a()
            }

            var y = "XOTQwMzc5Nzg4", w = $("#shelter"), g = $("#popVideo"), b = g.find(".video-wrapper"), I = g.find(".video-close"), k = $(document).find("button.play"), T = null, P = function () {
                this.init()
            };
            P.prototype = {
                init: function () {
                    $("#h5-video").show();
                    var h = (g.find(".video-js"), g.find("video")), c = '<source src="../../assets/video.webm" type="video/webm" /><source src="../../assets/video.mp4" type="video/mp4" /><source src="../../assets/video.flv" type="video/flv" />';
                    h.prepend(c), videojs.options.flash.swf = "video-js.swf", window.myPlayer = videojs("video_1", {techOrder: ["html5", "flash", "other supported tech"]}), myPlayer.ready(function () {
                        a(function () {
                            myPlayer.play()
                        })
                    })
            ...
            C.prototype = {
                wrapper: null,
                content: '<div id="youku" class="hide" style="width:944px;height:531px"></div>',
                youkuPlayer: null,
                init: function () {
                    this.wrapper = $("#youku-video"), this.wrapper.show();
                    this.youkuPlayer = new YKU.Player("youku", {
                        styleid: "1",
                        client_id: "66160027377018a6",
                        vid: y,
                        autoplay: !0,
                        show_related: !1
                    })
                ...
                v()
            }), 0 == h ? T = new C : 1 == h && (T = new C);
            var E = "paiIsPlay";
            $.cookie(E) || $.cookie(E, 1, {expires: 200})
        };
        $.ajax({url: "http://player.youku.com/jsapi", dataType: "script", cache: !0}).success(function () {
            h.init(), a.init(), c(document.createElement("video").canPlayType ? 0 : 1)
        })
    })
});
