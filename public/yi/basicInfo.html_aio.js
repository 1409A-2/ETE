...
/*!dep/jquery.mailAutoComplete.js*/
;
define("dep/jquery.mailAutoComplete", ["require", "exports", "module", "dep/jquery/dist/jquery"], function (require) {
    require("dep/jquery/dist/jquery"), function (c) {
        c.fn.mailAutoComplete = function (a) {
            var h = {
                className: "emailist",
                email: ["qq.com", "gmail.com", "126.com", "163.com", "hotmail.com", "yahoo.com", "yahoo.com.cn", "live.com", "sohu.com", "sina.com"],
                zIndex: 11
            }, v = c.extend({}, h, a || {}), b = "number" == typeof window.screenX, y = "visibility", g = {
                up: 38,
                down: 40,
                enter: 13,
                esc: 27,
                ...
                var h = "", b = a.value.split("@"), y = [];
                return c.each(v.email, function (c, a) {
                    (2 !== b.length || "" === b[1] || 0 === a.indexOf(b[1].toLowerCase())) && y.push(a)
                }), c.each(y, function (c, v) {
                    h = h + "<li" + (a.indexSelected === c ? ' class="on"' : "") + ">" + b[0] + "@" + v + "</li>"
                }), h
            }, w = function (ul, a) {
                var h = c.trim(this.value), v = "";
                "" === h || "" === (v = S(this)) ? ul.css(y, "hidden") : (a && (this.indexSelected = -1), ul.css(y, "visible").html(v))
            };
            return c(this).each(function () {
                this.indexSelected = -1;
                var a = this, h = c("<ul></ul>").css({
                    position: "absolute",
                    marginTop: a.offsetHeight + 3,
                    minWidth: a.offsetWidth - 3,
                    visibility: "hidden",
                    zIndex: v.zIndex
                }).addClass(v.className).bind("click", function (e) {
                    var h = e && e.target;
                    h && "li" === h.tagName.toLowerCase() && (c(a).val(h.innerHTML).trigger("input"), c(this).css(y, "hidden"), a.focus())
                });
                c(this).before(h), window.XMLHttpRequest || h.width(a.offsetWidth - 2), b ? c(this).bind("input", function () {
                    w.call(this, h, !0)
                }) : a.attachEvent("onpropertychange", function (e) {
                    "value" === e.propertyName && w.call(a, h, !0)
                }), c(document).bind({
                    click: function (e) {
                        var c = e && e.target, b = "";
                        c == a && a.value && (b = S(a, v.email)) ? h.css(y, "visible").html(b) : c != h.get(0) && c.parentNode != h.get(0) && h.css(y, "hidden")
                    }, keydown: function (e) {
                        var v = h.find("li");
                        if ("visible" === h.css(y)) {
                            switch (e.keyCode) {
                                case g.up:
                                    a.indexSelected--, a.indexSelected < 0 && (a.indexSelected = -1 + v.length), e.preventDefault && e.preventDefault();
                                    break;
                                case g.down:
                                    a.indexSelected++, a.indexSelected >= v.length && (a.indexSelected = 0), e.preventDefault && e.preventDefault();
                                    break;
                                case g.enter:
                                    e.preventDefault(), v.get(a.indexSelected) && c(a).val(v.eq(a.indexSelected).html()), h.css("visibility", "hidden");
                                    break;
                                case g.tab:
                                case g.esc:
                                    h.css("visibility", "hidden")
                            }
                            -1 !== a.indexSelected && h.html(S(a))
                        }
                    }
                })
            })
...
/*!talents/page/basicInfo/basicInfo.js*/
;
define("talents/page/basicInfo/basicInfo", ["require", "exports", "module", "common/static/js/common", "dep/jquery.mailAutoComplete", "dep/jquery-colorbox/jquery.colorbox-min"], function (require) {
    var a = require("common/static/js/common");
    require("dep/jquery.mailAutoComplete"), require("dep/jquery-colorbox/jquery.colorbox-min"), $(function () {
        function c(e) {
            if ($(".add-tip").hide(), e.preventDefault(), I.all() && j) {
                var a = [];
                $('input[name="expectCity"]:checked').each(function () {
                    a.push($(this).val())
                });
                var v = [];
                $('input[name="componySize"]:checked').each(function () {
                    v.push($(this).val())
                });
                var g = [];
                $(".mail-list span").each(function () {
                    g.push($(this).text())
                });
                var b = h.data.expectPosition;
                if (0 == b.length)return $(".radio").siblings(".error-tip").text("至少选择1项期望职位").css("visibility", "visible"), !1;
                $(".radio").siblings(".error-tip").css("visibility", "hidden");
                var y = $("#currentSalary").val(), k = $("#paidMonth").val();
                if (y || k) {
                    if (!y && k || y && !k)return y ? $("#paidMonth").next().text("请将发薪月数填写完整").css("visibility", "visible") : $("#currentSalary").next().text("请将当前薪资填写完整").css("visibility", "visible"), !1;
                    if ($("#currentSalary").val($("#currentSalary").val().replace(/\D|^0|/g, "")), !I.currentSalary($("#currentSalary")))return !1;
                    if ($("#paidMonth").val($("#paidMonth").val().replace(/\D|^0|/g, "")), !I.paidMonth($("#paidMonth")))return !1
                } else $(".current-salary").find(".error-tip").css("visibility", "hidden");
                $("#submit").off("click");
                var C = 0, w = $("#selectIdentity").html();
                "女士" == w && (C = 2), "先生" == w && (C = 1), $.ajax({
                    url: "beatPro",
                    data: {
                        name: $("#userName").val(),
                        sex: C,
                        property: $('input[name="field"]:checked').val(),
                        workYear: $("#selectYear").html(),
                        jobIntention: $("#selectIntention").html(),
                        companyName: $("#company").val(),
                        expectPosition: h.data.expectPosition.join(","),
                        expectCity: a.join(","),
                        companyStageContent: v.join(";"),
                        shieldContent: g.join(";"),
                        expectSalary: $("#salary-range-start").val() + "K-" + $("#salary-range-end").val() + "K",
                        phone: $("#phoneNumber").val(),
                        email: $("#email").val(),
                        currentSalary: $("#currentSalary").val(),
                        paidMonth: $("#paidMonth").val(),
                        resubmitToken: $("#resubmitToken").val(),
                        descript: $("#self-intro-content").val(),
                        code: $("#phone-verify-codes").val()
                    },
                    type: "POST",
                    dataType: "json",
                    success: function (a) {
                        1 == a.success ? window.location.href = ctx + "/talent/center.html" : (alert(a.message), $("#submit").on("click", function (e) {
                            c(e)
                        }))
                    },
                    error: function () {
                        alert("未知异常！请刷新后重试"), $("#form").get(0).reset()
                    }
                })
            }
            return !1
        ...
        var h = {
            init: function () {
                if ($(".radio").attr("data-expectPosition"))var a = $(".radio").attr("data-expectPosition").split(","); else var a = [];
                this.data = {expectPosition: a}
            }, findIndex: function (a) {
                var c = this.data.expectPosition;
                if (c) {
                    var i, h = c.length;
                    for (i = 0; h > i; i++)if (c[i] == a)return i;
                    return -1
                }
            }, add: function (a) {
                this.isBan() && this.data.expectPosition.push(a)
            }, remove: function (a) {
                var i = this.findIndex(a);
                -1 != i && this.data.expectPosition.splice(i, 1)
            }, removeAll: function () {
                this.data.expectPosition = []
            }, changeProfessional: function () {
                return this.data.expectPosition.length >= 1 ? !1 : !0
            }, isBan: function () {
                return this.data.expectPosition.length >= 3 ? !1 : !0
            }, banAll: function () {
                return this.data.expectPosition.length >= 3 ? !0 : !1
            }, getVal: function () {
                return this.data
            }
        };
        ...
        var v = function () {
            var a = $(this);
            if ("checkbox" === a.siblings("input").attr("type")) {
                a.hasClass("checked") ? a.hasClass("radio-expectPosition") && (a.removeClass("checked"), a.siblings("input").prop("checked", !0), h.remove(a.siblings("input").val())) : a.hasClass("radio-expectPosition") && (h.banAll() || (a.addClass("checked"), a.siblings("input").prop("checked", !1), h.add(a.siblings("input").val()), h.banAll()));
                var c = $(".radio").attr("data-property");
                $(".radio-content").eq(c - 1).find("label").each(3 == h.data.expectPosition.length ? function () {
                    $(this).hasClass("checked") || $(this).addClass("disabled")
                } : function () {
                    $(this).removeClass("disabled")
                })
            }
            I.expectPositionCheck()
        };
        $(".radio-content").find("label").on("click", v);
        var g = function () {
            var a = $(this), c = a.find('input[type="checkbox"]');
            c.is(":checked") ? (c.prop("checked", !1), $(".unexpectCompany-wrapper, .mail-list").slideUp(200), $(".mail-list").siblings(".error-tip").css("visibility", "hidden")) : (c.prop("checked", !0), $(".unexpectCompany-wrapper, .mail-list").slideDown(200))
        };
        $(".switch").on("click", g), $("input:radio:checked,input:checkbox:checked").each(function (a, c) {
            $(c).siblings("label").addClass("active")
        }), $(".select-input").each(function (a, c) {
            var c = $(c);
            c.val() && c.siblings(".select-title").html(c.val())
        });
        var b = $(document).height(), y = {
            show: function (a, c) {
                c = "undefined" == typeof c ? "" : c, a.parents(".input-wrapper").find("div.error-tip").html(c).css({visibility: "visible"});
                var h = a.parents(".input-wrapper").children("label:first")[0];
                h || (h = a.parents("#form").find("label:first")[0]);
                var v = $(h).offset().top;
                b = b > v ? v : b
            }, hide: function (a) {
                a.parents(".input-wrapper").find("div.error-tip").html("").css({visibility: "hidden"})
            }
        }, k = {
            show: function (a, c) {
                c = "undefined" == typeof c ? "" : c, a.parents(".salary-range").find("div.error-tip").html(c).css({visibility: "visible"});
                var h = a.parents(".salary-range").children("label:first")[0], v = h.offsetTop;
                b = b > v ? v : b
            }, hide: function (a) {
                a.parents(".salary-range").find("div.error-tip").css({visibility: "hidden"})
            }
        };
        $('input[type="text"]').on("focus", function () {
            y.hide($(this))
        });
        var C = $("#moreField"), w = C.nextAll("ul"), P = C.nextAll("i.select-triangle");
        C.on("click", function () {
            w.stop().toggle(), P.stop().toggleClass("active"), y.hide(C)
        }), P.on("click", function () {
            w.stop().toggle(), P.stop().toggleClass("active"), y.hide(C)
        }), w.on("click", "label", function () {
            var a = $(this);
            C.addClass("active"), C.html(a.html()), P.removeClass("active")
        }), $(".select-click").on("click", function () {
            "SPAN" == this.tagName && ($(this).toggleClass("active"), $(this).stop().siblings(".select-triangle").toggleClass("active").siblings(".select-list").stop().toggle()), "I" == this.tagName && ($(this).prev("span").toggleClass("active"), $(this).stop().toggleClass("active").siblings(".select-list").stop().toggle()), y.hide($(this))
        }), $(".select-list").on("click", "label", function () {
            var a = $(this);
            a.parents(".select-list").siblings(".select-title").html(a.html()).css("color", "#333").siblings(".select-input").val(a.html()).siblings(".select-click").removeClass("active").siblings(".select-wrapper-default").removeClass("select-wrapper-default")
        }), $(document).on("click", function (e) {
            $(".reserve-triangle-click").removeClass("active");
            var a = e.target;
            return a === C[0] || a === P[0] ? ($(".select-list").hide(), $(".select-click").removeClass("active"), !1) : $(a).hasClass("select-click") ? (w.hide(), P.removeClass("active"), !1) : (w.hide(), P.removeClass("active"), $(".select-list").hide(), void $(".select-click").removeClass("active"))
        }), $(".radio-wrapper").on("click", "label", function () {
            var a = $(this);
            if ("moreField" != a.attr("id") && !a.hasClass("active")) {
                h.removeAll(), $(".radio-content").find("label").removeClass("checked"), $(".radio-content").find("label").removeClass("disabled"), $(".radio-content").find('input[type="checkbox"]').prop("checked", !1), a.parents(".radio-wrapper").find("label").removeClass("active"), a.addClass("active");
                var c = a.parent("li").index();
                $(".radio-content").removeClass("active"), $(".radio-content").eq(c).addClass("active"), $(".radio").attr("data-property", c + 1), a.parents(".sub-wrapper").length > 0 && C.addClass("active"), y.hide(a)
            }
        }), $(".checkbox-wrapper").on("click", "label", function () {
            var a = $(this);
            a.hasClass("active") && a.addClass("unchecked"), a.stop().toggleClass("active"), y.hide(a)
        }).on("mouseleave", "label", function () {
            $(this).removeClass("unchecked")
        });
        var I = {
            fontColor: function (a, c) {
                var h = $.trim(a.text());
                h === c ? a.css("color", "#999") : a.css("color", "#333")
            }, radio: function (a) {
                var c = $("input[name=" + a + "]:checked");
                return c.length <= 0 ? (y.show($("input[name=" + a + "]"), "请选择"), !1) : (y.hide($("input[name=" + a + "]")), !0)
            }, checkbox: function (a) {
                var c = $("input[name=" + a + "]:checked");
                return c.length <= 0 ? (y.show($("input[name=" + a + "]"), "请选择"), !1) : (y.hide($("input[name=" + a + "]")), !0)
            }, name: function (a) {
                var c = $.trim(a.val());
                return "真实姓名" !== c && c ? c.length > 15 ? (y.show(a, "姓名不能超过15个字"), !1) : (y.hide(a), !0) : (y.show(a, "请填写姓名"), !1)
            }, position: function (a) {
                var c = $.trim(a.val());
                return "" == c ? (y.show(a, "请填写期望职位"), !1) : c.length > 15 ? (y.show(a, "期望职位不能超过15个字"), !1) : (y.hide(a), !0)
            }, phone: function (a) {
                var c = $.trim(a.val());
                return c ? /^1[3,4,5,7,8]{1}[0-9]{9}$/.test(c) ? (y.hide(a), !0) : (y.show(a, "目前仅支持11位有效中国大陆号码"), !1) : (y.show(a, "请填写手机号码"), !1)
            }, email: function (c) {
                var h = $.trim(c.val());
                return h ? h.length > 60 ? (y.show(c, "邮件地址太长"), !1) : a.isEmail(h) ? (y.hide(c), !0) : (y.show(c, "请填写正确的邮箱地址"), !1) : (y.show(c, "请填写邮箱地址"), !1)
            }, salary: function (a) {
                var c = parseInt($.trim(a.val())), h = parseInt($("#salary-range-start").val()), v = parseInt($("#salary-range-end").val());
                if (!c)return k.show(a, "需给出月薪上限和下限"), !1;
                if (!h)return !1;
                if (c > 100)return k.show(a, "请输入有效的月薪范围，1k-100k"), !1;
                if (h > v)k.show(a, "最高月薪需大于最低月薪"), flag = !1; else {
                    if (!(v > 2 * h))return k.hide(a), !0;
                    k.show(a, "最高月薪不能大于最低月薪的2倍"), flag = !1
                }
            }, currentSalary: function (a) {
                var c = /^(?:1|[1-9][0-9]?|100)$/, h = a.val(), v = $("#currentSalary").next();
                return c.test(h) ? (v.css("visibility", "hidden"), !0) : (v.text("当前薪资应在1~100范围之内").css("visibility", "visible"), !1)
            }, paidMonth: function (a) {
                var c = /^((1[2-9])|(2[0-4]))$/, h = a.val(), v = $("#paidMonth").next();
                return c.test(h) ? (v.css("visibility", "hidden"), !0) : (v.text("发薪月数应在12~24范围之内").css("visibility", "visible"), !1)
            }, select: function (a) {
                var c = "" === $.trim(a.val()) || "女士/先生" === $.trim(a.val()) || "请选择..." === $.trim(a.val());
                return c ? (y.show(a, "请选择"), !1) : (y.hide(a), !0)
            }, expectPositionCheck: function () {
                var a = h.data.expectPosition;
                0 === a.length ? $(".radio").siblings(".error-tip").text("至少选择1项期望职位").css("visibility", "visible") : $(".radio").siblings(".error-tip").css("visibility", "hidden")
            }, selfIntroContent: function (a) {
                var c = $.trim(a.val());
                return c ? c.length < 50 ? (y.show(a, "自我介绍不少于50字"), !1) : c.length > 200 ? void y.show(a, "自我介绍不超过200字") : (y.hide(a), !0) : (y.show(a, "请填写自我介绍"), !1)
            }, hasReserveTime: function (a) {
                var c = $.trim(a.val());
                return c ? (y.hide(a), !0) : (y.show(a, "请预约上场时间"), !1)
            }, verifyCodesCheck: function (a) {
                var c = $.trim(a.val());
                if ("验证码错误" == a.next().text()) {
                    var h = $("#phone-verify-codes").prev().offset().top;
                    return b = b > h ? h : b, j = !1, !1
                }
                return c ? c.length < 6 ? (y.show(a, "请输入6位数字验证码"), j = !1, !1) : (y.hide(a), !0) : (y.show(a, "请填写验证码"), j = !1, !1)
            }, all: function () {
                var a = this, c = !0;
                b = $(document).height(), a.name($("#userName")) || (c = !1), $.each(["field", "expectCity"], function (h, v) {
                    a.radio(v) || (c = !1)
                }), a.phone($("#phoneNumber")) || (c = !1), a.email($("#email")) || (c = !1), $(".select-input").each(function () {
                    a.select($(this)) || (c = !1)
                }), a.selfIntroContent($("#self-intro-content")) || (c = !1), a.verifyCodesCheck($("#phone-verify-codes")) || (c = !1), $.each([$("#salary-range-start"), $("#salary-range-end")], function (h, v) {
                    if (!a.salary($(v))) {
                        var g = $("#salary-range-start").offset().top;
                        b = b > g ? g : b, c = !1
                    }
                });
                var v = h.data.expectPosition;
                if (0 == v.length) {
                    c = !1, $(".radio").siblings(".error-tip").text("至少选择1项期望职位").css("visibility", "visible");
                    var g = $(".radio").offset().top;
                    b = b > g ? g : b
                }
                var y = $("#cmn-toggle").is(":checked"), k = [];
                if ($(".mail-list span").each(function () {
                        k.push($(this).text())
                    }), y && 0 == k.length) {
                    $(".mail-list").siblings(".error-tip").css("visibility", "visible"), c = !1;
                    var C = $(".shieldCompany").offset().top;
                    b = b > C ? C : b
                } else $(".mail-list").siblings(".error-tip").css("visibility", "hidden");
                return c || window.scrollTo(0, b), c
            }
        };
        (a.isIE8 || a.isIE9) && ($("#userName").on("focus", function () {
            "真实姓名" == $(this).val() && $(this).val("").css("color", "#333")
        }), $("#userName").on("blur", function () {
            "" == $(this).val() && $(this).val("真实姓名").css("color", "#999")
        })), $("#phoneNumber").on("keyup input change", function () {
            I.phone($(this))
        }), $("#userName").on("keyup input change", function () {
            I.name($(this))
        }), $("#email").on("keyup input change", function () {
            I.email($(this))
        }), $("#self-intro-content").on("keyup input change", function () {
            I.selfIntroContent($(this))
        });
        var j = !1, S = $("#phone-verify-codes");
        S.on("keyup input change blur", function () {
            var a = S.val();
            if ("验证码错误" != S.next().text())if (6 == a.length && 0 == j) {
                var c = $("#phoneNumber");

                        a.success ? (y.show(S, "验证码正确"), j = !1) : (y.show(S, "验证码正确"), j = !1)


            } else I.verifyCodesCheck(S)
        }), $("#salary-range-start, #salary-range-end").on("keyup paste", function () {
            $(this).val($(this).val().replace(/\D|^0/g, "")), 0 === parseInt($(this).val()) && $(this).val(""), I.salary($(this))
        }), $("#currentSalary").on("keyup input change blur", function () {
            $(this).val($(this).val().replace(/\D|^0|/g, "")), I.currentSalary($(this))
        }), $("#paidMonth").on("keyup input change blur", function () {
            $(this).val($(this).val().replace(/\D|^0|/g, "")), I.paidMonth($(this))
        }), $("#salary-range-start, #salary-range-end").css("ime-mode", "disabled"), I.fontColor($("#selectIdentity"), "女士/先生"), I.fontColor($("#selectYear"), "请选择..."), I.fontColor($("#selectIntention"), "请选择你的求职意向");
        $("#submit").on("click", function (e) {
            c(e)
        }), a.autoComplete("#expectPosition", 1), a.autoComplete("#company", 2), $(".tip-show").hover(function () {
            var c = $(this).offset().top - 86;
            if ("I" == this.tagName && ("selectIdentity" == $(this).prev("span").attr("id") ? c = $("#selectIdentity").offset().top - 86 : "selectYear" == $(this).prev("span").attr("id") && (c = $("#selectYear").offset().top - 86)), c != parseInt($(".m-tip").css("top"))) {
                var h = a.dataset($(this));
                a.isEmpty(h) || ($(".m-tip h3").html(h.tipTitle), $(".m-tip p").html(h.tipContent), $(".m-tip").hide(), $(".m-tip").css("top", c).fadeIn())
            }
        }, function () {
        });
        var T = $(".step-details"), A = T.next(".mail-list"), N = $("#addBox"), M = N.find("#unexpectCompany"), D = T.find(".add-tip"), B = T.find(".error-tip"), E = function (a) {
            return /^@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(a)
        }, Y = function (a) {
            return a = $.trim(a), "" === a ? (V("请填写正确的邮箱地址"), !1) : a.length > 32 ? (V("邮件后缀太长"), !1) : E(a) ? (D.show(), B.hide(), !0) : (V("请填写正确的邮箱地址"), !1)
        }, V = function (a) {
            a = a || "", D.hide(), B.css({color: "#f94e4e"}).html(a).show()
        }, _ = function (a) {
            13 == a.keyCode && N.find("#btn_ok").trigger("click")
        }, F = function () {
            N.hide(), D.hide(), B.hide(), $(document).off("keydown", _)
        }, K = function () {
            N.show(), M.focus(), B.hide(), D.html("请输入你想屏蔽企业的邮箱后缀").css({color: ""}).show(), $(document).on("keydown", _)
        };
        T.find(".add").on("click", function () {
            M.val("@"), K()
        }), $(M).on("keyup input change", function () {
            Y($(this).val())
        }), N.find("#btn_cancel").on("click", function () {
            F()
        }), N.find("#btn_ok").on("click", function () {
            var a = $(this).prev().val();
            return 0 == Y(a) ? !1 : (A.append($('<p><span style="color: #333;">' + a + '</span><input class="btn-delete" type="button" value="删除"/></p>')), F(), void($("p", A).length >= 10 && T.find(".add").hide()))
        }), A.on("click", ".btn-delete", function () {
            $(this).parent().remove(), T.find(".add").show()
        }), $(".inputMailList").mailAutoComplete(), $(".verify-btn").on("click", function () {
            var a = $(this), c = $("#phone-verify-codes").next(), h = $("#phoneNumber"), v = h.val(), g = I.phone(h);
            0 == g || "获取验证码" != a.text() && "重新发送" != a.text() || $.ajax({
                url: "/sendPhoneCode.json",
                data: {phone: v},
                type: "POST",
                dataType: "json",
                success: function (v) {
                    function $(a) {
                        return 0 == g ? (a.removeClass("disabled"), a.text("重新发送"), void(g = 60)) : (a.addClass("disabled"), a.text("重新发送(" + g + ")"), g--, void setTimeout(function () {
                            $(a)
                        }, 1e3))
                    }

                    if (v.success) {
                        if (1 != v.state)y.show(h, "目前仅支持11位有效中国大陆号码"); else if (1 == v.state) {
                            var g = 60;
                            a.hasClass("disabled") || $(a)
                        }
                    } else c.text(v.message).css("color", "red").show()
                },
                error: function () {
                    //c.text(v.message).css("color", "red").show()
                    console.log(ctx + "/sendPhoneCode.json调用出错")
                }
            })
        });
        var O = function (a) {
            var c = $(".m-progress .item:eq(" + (a - 1) + ")");
            c.children("i").css("background", "url(/yi/progress.png) 0 0 no-repeat"), c.prevAll(".item").children("i").css("background", "url(/yi/progress.png) 0 0 no-repeat"), c.prevAll(".arrow").css("background", "url(/yi/progress.png) 0 -38px no-repeat"), c.nextAll(".item").children("i").css("background", "url(/yi/progress.png) -38px 0 no-repeat"), c.nextAll(".arrow").css("background", "url(/yi/progress.png) 0 -52px no-repeat");
            var h;
        ...
        O(1);
        var W, z = $(".self-intro-demo");
        $(".self-intro-faq, .self-intro-demo").hover(function () {
            clearTimeout(W), z.show()
        }, function () {
            W = setTimeout(function () {
                z.hide()
            }, 500)
        });
        var L = $("#resume-schedule").val();
        ("true" == L || 1 == L) && $.colorbox({
            innerWidth: "502px",
            title: "简历不完善",
            overlayClose: !1,
            closeButton: !1,
            inline: !0,
            href: "#resume-imperfect"
        });
        var R = $("#phone-verify-codes");
        R.on("keyup", function () {
